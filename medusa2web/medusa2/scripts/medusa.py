...
#	logging.info('Input File: '+target+' --- Sha256: '+hashFile(target))
	...
	if options.skipMap == None and options.inputGraph == None:
		...
		if options.inputGraph == None:
			...
			logging.info(greenText('Option -s (--skipMap) selected. MUMmer .coords/.paf files will be searched in %s' %coordsDir))
			checkExistence(coordsDir)
			coords = [os.path.join(coordsDir,f) for f in os.listdir(coordsDir) if f.endswith('.coords') or f.endswith('.paf')]
		else:
			if options.skipMap != None and options.inputGraph != None:
